...
class SyndicateCard extends StatelessWidget {
  const SyndicateCard({super.key, required this.syndicateId, this.caption, this.trailing, required this.onTap});
      ...
      builder: (BuildContext context, SizingInformation sizing) {
        return InkWell(
          ...
          child: AppCard(
            ...
            child: Padding(
              padding: EdgeInsets.symmetric(vertical: (mediaQuery.size.longestSide / 100) * 1.5),
              child: ListTile(
                leading: SyndicateIcon(syndicate: syndicateName, iconSize: 50),
                title: Text(toBeginningOfSentenceCase(syndicateName.name) ?? syndicateName.name, style: titleStyle),
