...
abstract class ModPainter extends CustomPainter {
  ...
  void paintBacker(Canvas canvas) {
    ...
    if (drain != null) {
      ...
      final color = _rarity?.toColor();
      final textStyle = color != null ? TextStyle(color: Color(color)) : null;
      ...
      if (drain! < 0) {
        textPainter.paint(canvas, const Offset(218, 102));
        return;
      }

      textPainter.paint(canvas, const Offset(218, 101));
    }
  ...
  void paintDescription(Canvas canvas, Size size) {
    ...
    final span = TextSpan(
      text: description,
      style: color != null ? TextStyle(color: Color(color), fontSize: 12) : null,
    );
  ...
  void paintPolarity(Canvas canvas, Rect rect) {
    ...
    paintImage(
      ...
      colorFilter: ColorFilter.mode(color != null ? Color(color) : Colors.white, BlendMode.srcIn),
    );
  }
