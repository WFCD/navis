...
class Inventoria {
  /// {@macro inventoria}
  Inventoria([Client? client]) : _client = client ?? Client(), _database = _InventoriaDatabase();

  final Client _client;
  final _InventoriaDatabase _database;
  ...
  Future<void> updateProfile(String id) async {
    ...
    await _database.computeWithDatabase(
      connect: _InventoriaDatabase.new,
      computation: (d) async {
        ...
        await d.transaction(() async {
          for (final update in updates) {
            ...
            await (d.update(d.inventoryItem)..whereSamePrimaryKey(
              item,
            )).write(InventoryItemData(uniqueName: item.uniqueName, name: item.name, type: item.type, xp: update.xp));
          }
        });
      },
    );
  ...
  Future<void> updateInventory() async {
    ...
    InventoryItemCompanion toInsert(Item item) {
      return InventoryItemCompanion.insert(
        uniqueName: item.uniqueName,
        name: item.name,
        description: Value.absentIfNull(item.description),
        image: Value.absentIfNull(item.imageName),
        productCategory: Value.absentIfNull(item.productCategory),
        type: item.type.name,
      );
    }
    ...
    final newItems = items.where((i) => !currentManifest.contains(i.uniqueName)).map(toInsert);
...
class _InventoriaDatabase extends _$_InventoriaDatabase {
  /// {@macro inventoria}
  _InventoriaDatabase([QueryExecutor? executor]) : super(executor ?? driftDatabase(name: 'inventoria'));
